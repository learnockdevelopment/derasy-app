...
import '../../views/widgets/loading_prompts.dart';
...
class _VerifyEmailPageState extends State<VerifyEmailPage> {
  ...
  Future<void> _sendCode() async {
    ...
    try {
      ...
      // Debug logging
      print('📧 [VERIFY EMAIL] Response data: $response');
      print('📧 [VERIFY EMAIL] All response keys: ${response.keys.toList()}');
      print('📧 [VERIFY EMAIL] DevCode from response: ${response['devCode']}');
      print('📧 [VERIFY EMAIL] Code from response: ${response['code']}');
      print('📧 [VERIFY EMAIL] Message from response: ${response['message']}');
      print('📧 [VERIFY EMAIL] DevCode from model: ${verifyResponse.devCode}');
      ...
      if (devCode == null) {
        print(
            '📧 [VERIFY EMAIL] ⚠️ No verification code found in API response');
        print(
            '📧 [VERIFY EMAIL] Available fields: ${response.keys.join(', ')}');

        // For development purposes, generate a mock code
        devCode = '123456';
        print(
            '📧 [VERIFY EMAIL] 🔧 Using mock verification code for development: $devCode');
      }
      ...
      if (verifyResponse.sent) {
        ...
        if (devCode != null) {
          print('📧 [VERIFY EMAIL] 📧 EMAIL VERIFICATION CODE: $devCode');
        }
      }
    } catch (e) {
      int? statusCode;
  ...
  Future<void> _verifyCode() async {
    ...
    LoadingPrompts.showVerifyCodeLoading();
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 8.h),
                    Text(
                      'we_sent_verification_code_email'.tr,
