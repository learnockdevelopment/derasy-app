...
class _ClinicRecordsPageState extends State<ClinicRecordsPage> {
  ...
  Future<void> _loadClinicRecords() async {
    ...
    try {
      ...
      Get.snackbar(
        'Error',
        'Failed to load clinic records: ${e.toString()}',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(
          'Clinic Records - ${widget.student.fullName}',
    ...
    return Center(
      child: Column(
        children: [
          ...
          SizedBox(height: 8.h),
          Text(
            'No clinic records found for this student',
            style: AppFonts.bodyMedium.copyWith(
              color: AppColors.textSecondary,
            ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (record.symptoms?.isNotEmpty == true)
            _buildInfoSection('Symptoms', record.symptoms!, Icons.warning),

          // Diagnosis
          if (record.diagnosis?.isNotEmpty == true)
            _buildInfoSection(
                'Diagnosis', record.diagnosis!, Icons.medical_services),
          ...
          if (record.medication?.isNotEmpty == true)
            _buildInfoSection(
                'Medication', record.medication!, Icons.medication),

          // Follow-up
          if (record.followUp?.isNotEmpty == true)
            _buildInfoSection('Follow-up', record.followUp!, Icons.schedule),
        ],
      ),
    );
  }
